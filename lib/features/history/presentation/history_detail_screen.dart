  ...
  Future<void> _submitRating(int rating) async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('평가 저장 중 오류가 발생했습니다: $e'),
                          ...
                          return GestureDetector(
                            ...
                            child: Padding(
                              ...
                              child: Icon(
                                ...
                                size: 40,
                                color: _isRatingLoading ? Colors.grey : Colors.amber,
                              ),
                            ),
                          );
                        }),
  ...
  Future<void> _toggleLike(Product product) async {
    ...
    try {
      ...
      setState(() {
        ...
        final idx = _history.recommendations
            .indexWhere((p) => p.productId == product.productId);
        if (idx != -1) {
          _history.recommendations[idx] =
              _history.recommendations[idx].copyWith(liked: !wasLiked);
        }
      });
    } catch (e) {
